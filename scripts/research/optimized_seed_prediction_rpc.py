#!/usr/bin/env python3
"""
Optimierte Seed-basierte Vorhersage mit RPC-Validierung
- Testet nur vielversprechende Seed-Positionen
- Progress-Anzeige
- RPC-Validierung mit venv-tx
...
# Vielversprechende Seed-Positionen (basierend auf vorherigen Analysen)
PROMISING_SEED_POSITIONS = {
 27: [0, 4, 13, 27, 30, 54], # Position 27: bekannte gute Seed-Positionen
 13: [0, 4, 13, 27, 30, 54], # Position 13
 41: [0, 4, 13, 27, 30, 54], # Position 41
 55: [0, 4, 13, 27, 30, 54] # Position 55
}
...
def test_optimized_prediction(layer3_data: List[Dict], target_pos: int, matrix, rpc_enabled: bool = False, sample_size: int = 2000) -> Dict:
 """Optimierter Test mit RPC-Validierung."""
 
 print(f" 🔍 Position {target_pos}...")
 
 # 1. Teste nur vielversprechende Seed-Positionen
 seed_positions = PROMISING_SEED_POSITIONS.get(target_pos, list(range(55)))
 seed_results = {}
 
 print(f" Teste {len(seed_positions)} Seed-Positionen...")
 for i, seed_pos in enumerate(seed_positions):
 if (i + 1) % 3 == 0:
 print(f" {i + 1}/{len(seed_positions)}...")
 ...
 print(f" ✅ {len(best_seed_positions)} beste gefunden")
 ...
 print(f" ✅ RPC: {rpc_results['valid']}/{rpc_results['tested']} valid, {rpc_results['on_chain']} on-chain")
...
def main():
 ...
 print("=" * 80)
 print("OPTIMIERTE SEED-BASIERTE VORHERSAGE MIT RPC-VALIDIERUNG")
 print("=" * 80)
 print()
 ...
 print("⚠️ venv-tx nicht gefunden - RPC-Validierung abovesprungen")
 print()
 ...
 result = test_optimized_prediction(
 layer3_results,
 pos,
 matrix,
 ...
 if rpc["tested"] > 0:
 print(f" RPC: {rpc['valid']}/{rpc['tested']} valid, {rpc['on_chain']} on-chain")
 ...
 output_file = OUTPUT_DIR / "optimized_seed_prediction_rpc_results.json"
