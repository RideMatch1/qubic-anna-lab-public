...
All Anna Matrix identities are 60 uppercase letters. This script tries several
transformations (body, lowercase, trimming) and records why each candidate fails
the seed criteria. The goal is to document that the matrix strings behave like
identity bodies, not private seeds.

**Seed-to-Identity Derivation Test**
For each extracted string, the script tests if it can function as a seed by
deriving an identity from it and checking that identity on-chain.

Usage:
    python scripts/core/seed_candidate_scan.py                    # basic check
    python scripts/core/seed_candidate_scan.py --test-derivation # full on-chain test

Outputs:
    - outputs/reports/seed_candidate_scan.md
    - outputs/reports/seed_candidate_scan.json
"""
...
def _is_seed_like(text: str) -> bool:
    ...
    Qubic seeds need to be exactly 55 lowercase letters. No numbers, no uppercase.
    """
...
def _seed_to_private_key_bytes(seed: str) -> bytes:
    ...
    This is how Qubic derives private keys from seeds. The seed gets
    converted to bytes, then hashed to get the actual private key.
    Note: We found valid seeds in the matrix, not private keys directly.
    """
...
def _public_key_to_identity_body(public_key: bytes) -> str:
    """
    Convert 32-byte public key to Base-26 identity body.
    
    Reverses the process in identity_tools._pack_body.
    """
...
def analyse_identity(label: str, identity: str, test_seed_derivation: bool = True) -> CandidateReport:
    ...
    if len(identity) != IDENTITY_LENGTH or not identity.isalpha() or not identity.isupper():
        observations.append(f"Identity deviates from standard {IDENTITY_LENGTH}-char uppercase form.")
    else:
        observations.append(f"Identity is {IDENTITY_LENGTH} uppercase letters (standard public key format).")
    ...
    if _is_seed_like(identity):
        ...
        observations.append(f"Identity fails seed constraints (needs {SEED_LENGTH} lowercase letters).")

    if _is_seed_like(body):
        observations.append("Body matches seed constraints directly (unexpected).")
    else:
        observations.append(f"Body fails seed constraints ({IDENTITY_BODY_LENGTH} chars, uppercase).")

    body_lower = body.lower()
    chopped = body_lower[:SEED_LENGTH]
    if _is_seed_like(body_lower):
        observations.append(f"Lowercased body ({IDENTITY_BODY_LENGTH} chars) still too long; trimming required.")
    if _is_seed_like(chopped):
        observations.append(f"Truncated lowercased body ({SEED_LENGTH} chars) is seed-like; requires further manual review.")
    else:
    ...
    if test_seed_derivation and _is_seed_like(chopped):
        ...
        if derived_id:
            ...
            if status == "exists":
                observations.append(f"**NON-ZERO BALANCE:** Derived identity `{derived_id}` exists on-chain with balance {balance} QU!")
...
def write_reports(reports: Sequence[CandidateReport]) -> None:
    ...
    lines = [
        ...
        "",
        f"Qubic seeds must be **{SEED_LENGTH} lowercase letters (a-z)**. The Anna Matrix identities are {IDENTITY_LENGTH} uppercase letters.",
        "Documents the checks performed to determine whether the extracted strings could double as seeds.",
        "",
        "## Seed-to-Identity Derivation Test",
        "",
    ...
    for report in reports:
        ...
        if report.seed_derivation:
            ...
            if report.seed_derivation.rpc_balance and report.seed_derivation.rpc_balance != "0":
                rpc_status = f"**NON-ZERO BALANCE: {report.seed_derivation.rpc_balance} QU**"
        else:
    ...
    JSON_PATH.write_text(json.dumps([r.to_dict() for r in reports], indent=2), encoding="utf-8")  # pretty print

def main() -> None:
    """Main entry point."""
    import sys
    
    # check if we should test seed derivation (needs QubiPy, docker recommended)
    ...
    # Test all 8 identities (4 diagonal + 4 vortex)
    ...
    # Check for non-zero balances (would be interesting if found)
    non_zero = [r for r in reports if r.seed_derivation and r.seed_derivation.rpc_balance and r.seed_derivation.rpc_balance != "0"]
    if non_zero:
        print(f"\n⚠️  NON-ZERO BALANCE: Found {len(non_zero)} seed(s) with non-zero balance!")
        for r in non_zero:
            print(f"   {r.label}: {r.seed_derivation.rpc_balance} QU")
    elif test_derivation:
        print(f"\n[seed-scan] All derived identities checked. See report for details.")
